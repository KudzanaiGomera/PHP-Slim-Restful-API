...
$app->post('/personal','personal'); /* User Info  */
    ...
    try {
        ...
        if (strlen(trim($username))>0 && strlen(trim($password))>0 && strlen(trim($email))>0 && $email_check>0 && $username_check>0 && $password_check>0)
        {
            ...
            if($mainCount==0)
            {
                ...
                $sql1="INSERT INTO users(username,password,email,name,surname,mobile,id,gender,address,province, language)VALUES(:username,:password,:email,:name,:surname,:mobile,:id,:gender,:address,:province, :language)";
...
function internalUserDetails($input) {
    ...
    try {
        $db = getDB();
        $sql = "SELECT user_id, name, email, username, surname, mobile, id, gender, address, province,language FROM users WHERE username=:input or email=:input";
        $stmt = $db->prepare($sql);
...
function personal() {
    ...
    $medicalaidcondition=$data->medicalaidcondition;
    $bloodgroup=$data->bloodgroup;
    $sanbsnumber=$data->sanbsnumber;
    ...
    try {
        ...
        if (strlen(trim($username))>0 && strlen(trim($password))>0 && strlen(trim($email))>0 && $email_check>0 && $username_check>0 && $password_check>0)
        {
            ...
            if($mainCount==0)
            {
                ...
                /*Inserting user values*/
                $sql1="INSERT INTO personal(username,password,email,name,surname,mobile,id,gender,age, address,province, medicalaidnumber, primarymember, medicalaidcondition, bloodgroup, sanbsnumber, donornumber, allergies)VALUES(:username,:password,:email,:name,:surname,:mobile,:id,:gender, :age, :address,:province, :medicalaidcondition, :primarymember, :medicalaidcondition, :bloodgroup, :sanbsnumber, :donornumber, :allergies)";
                $stmt1 = $db->prepare($sql1);
                $stmt1->bindParam("username", $username,PDO::PARAM_STR);
                $password=hash('sha256',$data->password);
                $stmt1->bindParam("password", $password,PDO::PARAM_STR);
                ...
                $stmt1->bindParam("address", $address,PDO::PARAM_STR);
                $stmt1->bindParam("province", $province,PDO::PARAM_STR);
                $stmt1->bindParam("medicalaidnumber", $medicalaidnumber,PDO::PARAM_STR);
                $stmt1->bindParam("primarymember", $primarymember,PDO::PARAM_STR);
                $stmt1->bindParam("medicalaidcondition", $medicalaidcondition,PDO::PARAM_STR);
                $stmt1->bindParam("bloodgroup", $bloodgroup,PDO::PARAM_STR);
                $stmt1->bindParam("sanbsnumber", $sanbsnumber,PDO::PARAM_STR);
                $stmt1->bindParam("donornumber", $donornumber,PDO::PARAM_STR);
                $stmt1->bindParam("allergies", $allergies,PDO::PARAM_STR);
                $stmt1->execute();
